-- Apply Review Page Fixes
-- This script applies the migration to fix the review page issues
...
GRANT EXECUTE ON FUNCTION public.get_all_reviews_for_dashboard() TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_review_stats_for_dashboard() TO authenticated;

-- Test the setup with a sample insert/delete
DO $$ 
BEGIN
  -- Insert a test review (this should work for anonymous users)
  INSERT INTO public.reviews (name, phone, rating, country_code, google_review, redirect_opened)
  VALUES ('Test User', '+1234567890', 5, 'US', false, false);
  
  -- Delete the test review
  DELETE FROM public.reviews WHERE name = 'Test User' AND phone = '+1234567890';
  
  RAISE NOTICE 'Review page fixes applied successfully!';
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'Error applying fixes: %', SQLERRM;
END $$;
