...
DROP TABLE IF EXISTS public.user_invitations CASCADE; -- Legacy table, will be removed
...
-- Note: User invitations table removed - using Supabase Magic Link instead
...
-- User invitation indexes removed - using Supabase Magic Link instead
...
RETURNS TRIGGER AS $$
BEGIN
  -- Create profile for new user with metadata from magic link
...
-- user_invitations table removed - using Supabase Magic Link instead
...
-- User invitation policies removed - using Supabase Magic Link instead
...
-- create_tenant_with_admin function removed - using MagicLinkService instead
