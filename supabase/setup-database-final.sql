...
-- This version only drops objects that exist

-- Enable necessary extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Drop existing triggers only if they exist (checking table existence first)
DO $$
BEGIN
    ...
    -- Drop public table triggers only if tables exist
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'business_settings' AND table_schema = 'public') THEN
        DROP TRIGGER IF EXISTS handle_business_settings_insert_trigger ON public.business_settings;
...
DROP FUNCTION IF EXISTS public.get_user_business_settings() CASCADE;
DROP FUNCTION IF EXISTS public.get_user_reviews() CASCADE;
DROP FUNCTION IF EXISTS public.get_user_review_stats() CASCADE;
