-- Multi-Tenant Public Review URL System Migration (Safe Version)
-- Date: January 18, 2025
-- Purpose: Implement slug-based public review URLs with anonymous submissions
-- This version safely handles existing policies and structures
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_tenants_business_name') THEN
        CREATE INDEX idx_tenants_business_name ON public.tenants(business_name);
    END IF;
END $$;
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_tenant_users_tenant_id') THEN
        CREATE INDEX idx_tenant_users_tenant_id ON public.tenant_users(tenant_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_tenant_users_user_id') THEN
        CREATE INDEX idx_tenant_users_user_id ON public.tenant_users(user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_tenant_users_role') THEN
...
-- Make user_id nullable for anonymous submissions (only if it's currently NOT NULL)
DO $$ 
BEGIN
    IF EXISTS (
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_reviews_tenant_id') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_reviews_is_anonymous') THEN
        CREATE INDEX idx_reviews_is_anonymous ON public.reviews(is_anonymous);
    END IF;
END $$;
...
-- ============================================================================
-- 5. SAFELY UPDATE RLS POLICIES
-- ============================================================================

-- Drop existing policies that might conflict (with error handling)
DO $$ 
BEGIN
    ...
    -- Drop policies on reviews table
    ...
    -- Drop policies on tenant_users table
...
-- Apply trigger to tenant_users table (only if it doesn't exist)
DO $$ 
BEGIN
    IF NOT EXISTS (
